...
- The exit is judged by the same auction logic as the entry. A TP parked tick-tight at a reference (exit.tp1_vs_reference.band "parked_at"), especially a weekly value edge, is asking the auction for exactness — tp1_missed_by_pts of a point or two with post-exit travel left is a placement finding, not bad luck. The payout of a with-momentum trade is the runway to the next reference (ON high/low, PDH/PDL) — name it when the trade was cut far short of it. And an exit at the second successful defense of the entry — price held, momentum intact, then flat — gave up the trade at exactly the moment the market re-proved it. Terrain (exit.tp_terrain / exit.stop_terrain, from the session profile as of entry): a TP that must cross a thin gap and land in an LVN is asking price to accept where nobody was doing business — and stop_terrain.inside_entry_node true means the stop never left the volume of the range the entry traded from — in the noise, a donation — UNLESS the level facts show it sat behind structure that had to hold (a fresh rejection, a zone edge): judge the stop from BOTH facts together, the way the 08-19 stop was inside the node yet behind the PWL rejection and took zero heat. A stop beyond the node edge is the ticket priced at the structure. CONTEXT.htf_alignment and CONTEXT.runway are the bigger auction: say where the entry sat in the week's value when it changes the judgment, and when the trade banked a fraction of the runway to the next reference, name the reference and both numbers.
