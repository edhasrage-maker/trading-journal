...
export default async function DashboardPage() {
  ...
  const [{ data: tradesRaw }, { data: contextsRaw }] = dayIds.length > 0
    ? await Promise.all([
        supabase
          .from('trades')
          .select('id, trading_day_id, entry_time, tags_json, pnl, direction, entry_price, high_during_position, low_during_position, quantity, symbol')
  ...
  const recentDays = recentDaysBase.map(d => {
    ...
    // Live ATR averaged across the day's trades — replaces prep_atr for
