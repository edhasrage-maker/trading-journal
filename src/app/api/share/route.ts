...
export async function POST(req: Request) {
  // Cloud-only: the `shares` table exists only on the multi-tenant public DB.
  // In local (single-user) mode the Share button is hidden; guard the route too.
  if (LOCAL_FEATURES_ENABLED) {
    return NextResponse.json({ error: 'Sharing is not available in local mode.' }, { status: 404 })
  }
