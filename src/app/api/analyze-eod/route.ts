...
**Process layer (binary, per-rule):**
- For each of P1..P7, mark status as "pass", "fail", or "incomplete".
- Per v1.3 §Unscorable: P1-P6 incomplete = breach (these are safety rails);
  P7 incomplete = "incomplete" (data-completeness flag), NOT a fail.
...
- Verdict = "Compliant" ONLY if every rule is "pass" (P7 "incomplete" tolerated).
  Otherwise "Breach". P&L does not override.

**Execution layer (continuous, diagnostic, compliant trades only):**
- Score each sub-metric on 0..1 (higher = better):
    - duration_to_thesis (weight 35%): did the trade reach its thesis in
      reasonable time? Drawn-out chop = lower. Quick clean resolution = higher.
    - mfe_capture (weight 30%): realized PnL ÷ peak favorable move. Use
      high_during_position / low_during_position if provided per trade.
    - mae_heat (weight 20%): 1 - (peak adverse / planned risk). Lower heat
      taken = higher score.
    - planned_vs_realized_rr (weight 15%): realized_rr ÷ reward_ratio
      (when both available).
- Composite = 0.35*duration + 0.30*mfe + 0.20*mae + 0.15*rr. Null any
  sub-metric you can't compute; if all are null, composite is null.
...
Be direct. If the day was a Breach, say so plainly — don't soften it with "but the PnL was good." If the day was Compliant with poor execution, name that too — process compliance doesn't excuse sloppy execution. Magnitude doesn't matter for process; even a +$50 breach is still a breach.` : `Respond with ONLY valid JSON in this exact structure (no markdown, no code fences):
{
