...
export default function IntradayClient({ date, initialTrades, allTags: initialAllTags, initialOpenTradeId, prepDayTypes, initialSessionNotes = '', initialSessionEndedAt = null }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {trades.length > 0 && (
        <div className="border-t border-gray-800 pt-3 flex items-center gap-8 flex-wrap">
          <div>
            <div className="text-[11px] text-gray-500">Trades</div>
            <div className="text-[19px] font-bold tabular-nums text-gray-100" style={{ fontFamily: 'var(--font-display)' }}>{trades.length}</div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">Day P&L</div>
            <div className={`text-[19px] font-bold tabular-nums ${pnlColor(totalPnl)}`} style={{ fontFamily: 'var(--font-display)' }}>
              {`${totalPnl >= 0 ? '+' : '−'}$${Math.abs(totalPnl).toFixed(2)}`}
            </div>
          </div>
          <div>
            <div className="text-[11px] text-gray-500">Wins / Losses</div>
            <div className="text-[19px] font-bold tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>
      ...
      {effectiveChartSymbol && (
        <div className="border-t border-gray-700 pt-5">
          ...
          >
            <span className="text-base font-bold tracking-tight text-gray-100" style={{ fontFamily: 'var(--font-display)' }}>Chart</span>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 bg-gray-900 border border-gray-700 rounded-lg shadow-[0_24px_60px_-30px_rgba(0,0,0,0.9)] px-4 py-2 flex items-center gap-3 text-sm">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-blue-300 bg-blue-500/[0.08] border border-blue-500 hover:bg-blue-500/[0.14] px-3 py-1.5 rounded text-xs font-semibold transition-colors"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-gray-300 border border-gray-700 hover:text-gray-100 hover:border-gray-600 disabled:opacity-50 px-3 py-1.5 rounded text-xs font-semibold transition-colors"
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-red-400 border border-red-700 hover:bg-red-400/[0.08] disabled:opacity-50 px-3 py-1.5 rounded text-xs font-semibold transition-colors"
