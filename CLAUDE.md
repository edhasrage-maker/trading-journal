...
- **Process vs Grade scoring — resolved by Ruleset v1.3 (2026-06-08, twice amended same day).** Architecture: `/api/analyze-prep` produces `ai_analysis_json.score` = **Prep Quality** (renamed in UI from "Process" → "Prep Quality"; storage field `process_score` unchanged). `/api/analyze-eod` produces two orthogonal outputs nested in `eod_ai_analysis_json.process` (verdict + per-rule P1–P7) and `.execution` (5 sub-metrics + composite). v1.3 amendments 2026-06-08: (1) verdict threshold relaxed from "all 7 pass" → "5 of 7 pass" (P7 incomplete still tolerated); (2) execution gains a 5th `prep_adherence` sub-metric (20% weight, rebalanced from duration 35→25, MFE 30→25, RR 15→10). Dashboard: Grade column renamed to **Execution** (composite × 10, Math.round'd); Process column shows v1.3 verdict as banded color pill (green for Compliant 5–7/7 with shade gradient, red for Breach 0–4/7 with shade gradient); old "Process" column (which actually was Prep score) dropped from the dense Recent Days view. Spec file `docs/Ruleset_v1.3_Process_Execution_Spec.md` is canonical and loaded verbatim into the EOD prompt at module scope. EOD `max_tokens` is 6000 (was 2000, truncated v1.3 responses mid-string); prompt includes a LENGTH DISCIPLINE block to keep per-rule reasons short; parser strips markdown code fences as a defense. Trade block now includes TP1 + exit_price so the AI can compute planned_vs_realized_rr.
