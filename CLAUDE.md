...
- **Process vs Grade scoring — resolved by Ruleset v1.3 (2026-06-08).** Architecture is now: `/api/analyze-prep` produces `ai_analysis_json.score` = **Prep Quality** (renamed in UI from "Process" → "Prep Quality"; the data field name `process_score` is kept for storage-layer compat but the user-facing label changed). `/api/analyze-eod` produces TWO orthogonal outputs nested in `eod_ai_analysis_json`: `process: ProcessVerdict` (binary Compliant/Breach + per-rule P1–P7 status; any one breach = Breach) and `execution: ExecutionScore` (continuous 0..1 composite of 4 sub-metrics, computed only over compliant trades, NEVER blended with the verdict). The legacy single `score` field is preserved on pre-v1.3 rows; UI prefers the new structure when present. Canonical spec: `docs/Ruleset_v1.3_Process_Execution_Spec.md` — loaded verbatim into the EOD prompt at module scope so Claude grades against it. Don't change the rubric without updating the markdown file first.
